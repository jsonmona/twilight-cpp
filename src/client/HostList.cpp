...
HostList::Entry::Entry() :
	lastConnected(std::chrono::system_clock::from_time_t(0))
{
...
HostList::HostList() :
	log(createNamedLogger("HostList"))
{
}
...
bool HostList::load(toml::array arr) {
	...
	for (toml::value& now : arr) {
		...
		if (dir["server_cert"].is_string()) {
			...
			int ret = mbedtls_x509_crt_parse(&entry->serverCert,
		...
		if (dir["client_cert"].is_string()) {
			...
			int ret = mbedtls_x509_crt_parse(&entry->clientCert,
				reinterpret_cast<const unsigned char*>(pem.c_str()), pem.size() + 1);
			if (ret != 0)
				warned = warnInvalidCert = true;
		}
