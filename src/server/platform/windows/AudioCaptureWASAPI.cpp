...
AudioCaptureWASAPI::AudioCaptureWASAPI() :
	log(createNamedLogger("AudioCaptureWASAPI"))
{
}
...
void AudioCaptureWASAPI::runRecord_() {
	...
	hr = audioClient->Initialize(AUDCLNT_SHAREMODE_SHARED, AUDCLNT_STREAMFLAGS_LOOPBACK, bufferTime, 0, &waveformat.Format, nullptr);
	...
	int sizeofSample =
		waveformat.Format.wBitsPerSample == 8 ? 1 :
		waveformat.Format.wBitsPerSample == 16 ? 2 :
		waveformat.Format.wBitsPerSample == 32 ? 4 :
		waveformat.Format.wBitsPerSample == 64 ? 8 : 0;
	check_quit(av_fmt == AV_SAMPLE_FMT_NONE || sizeofSample == 0, log, "Failed to determine appropriate representation of waveformat");
